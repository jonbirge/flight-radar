...
import { lookupAirport } from '../data.js';
import { applyTheme } from './core.js';
import { renderAircraft, toggleAircraft, pollStates, startPolling } from './aircraft.js';
import { initAirports, initSmallAirports, removeSmallAirports, toggleAirports, initAirspace, rebuildAirspace, toggleAirspace, initNavaids, initFixes, removeFixes, toggleNavaids } from './markers.js';
import { enableRadar, disableRadar, refreshRadar, enableSatelliteIR, disableSatelliteIR, refreshSatelliteIR, enableTurbForecast, disableTurbForecast, refreshTurbForecast, computeTurbLevel, fetchSigmets, removeSigmetEntities, fetchAirmets, removeAirmetEntities, fetchPireps, removePirepEntities } from './weather.js';
...
export async function loadDataJSON(path) {
...
export async function loadAndApplySettings() {
  try {
    ...
    if (saved) {
      ...
      if (S.radarLayer) S.radarLayer.alpha = wxAlpha;
      if (S.satelliteIRLayer) S.satelliteIRLayer.alpha = wxAlpha;
      if (S.turbLayer) S.turbLayer.alpha = wxAlpha;
      ...
      if ((prevEdges !== CONFIG.airspaceEdges || prev3D !== CONFIG.airspace3D || prevExAlt !== CONFIG.exaggerateAltitudes) && S.airspaceEntities.length > 0) {
        rebuildAirspace();
      }
      ...
      if (prevSmallAirports !== CONFIG.showSmallAirports && S.cachedAirportData) {
        if (CONFIG.showSmallAirports) {
          initSmallAirports(S.cachedAirportData);
        } else {
          removeSmallAirports();
        }
      }
      ...
      if (prevNavaids !== CONFIG.navaidsEnabled) {
        if (CONFIG.navaidsEnabled && S.navaidEntities.length === 0 && S.cachedWaypointData) {
          initNavaids();
        }
        toggleNavaids(CONFIG.navaidsEnabled);
      }
      if (prevShowFixes !== CONFIG.showFixes && S.cachedWaypointData) {
        if (CONFIG.showFixes) {
          initFixes();
        } else {
          removeFixes();
  ...
  if (S.airportEntities.length === 0 && airports) {
    initAirports(airports);
  }

  // Initialize airspace boundaries
  if (S.airspaceEntities.length === 0 && airspace) {
    initAirspace(airspace);
  }

  // Cache waypoint data (entities created on-demand when enabled)
  if (waypoints) {
    S.cachedWaypointData = waypoints;
    if (CONFIG.navaidsEnabled && S.navaidEntities.length === 0) {
      initNavaids();
    }
  }
