...
import { lookupAirportCoords, estimateArrivalTime } from './flightplan.js';
import { filterAllWeather, getSelectedAircraftFL, pauseWeatherRefresh, resumeWeatherRefresh, fetchAirmetsForScrubbing, removeScrubAirmetEntities, preloadTurbForTimeline, getTurbPixelColor, clearTurbCache, updateTurbForTimelineTime, resetTurbToLive, fetchSigmets, removeSigmetEntities, fetchAirmets, removeAirmetEntities, fetchPireps, removePirepEntities } from './weather.js';
...
function interpolateRoutePosition(fraction) {
  const pts = S.timelineRoutePoints;
...
export function showTimeline(flight) {
...
function resetTimelineToLive() {
  ...
  if (S.timelineEntity) {
    S.viewer.entities.remove(S.timelineEntity);
    S.timelineEntity = null;
  }
}
...
export function enterScrubbingMode() {
...
function updateOrCreateTimelineMarker(position) {
  if (S.timelineEntity) {
    S.timelineEntity.position = position;
  } else {
    S.timelineEntity = S.viewer.entities.add({
      id: 'timeline-marker',
      position: position,
      point: {
        pixelSize: 14,
