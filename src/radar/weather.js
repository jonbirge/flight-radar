// Weather overlays — NEXRAD radar, GOES IR, AWC turbulence, PIREPs, SIGMETs, G-AIRMETs.
...
export function makeSatelliteIRProvider() {
...
export function addSatelliteIRLayer() {
  const provider = makeSatelliteIRProvider();
  // Insert below turbulence and radar layers
  if (S.turbLayer) {
    const idx = S.viewer.imageryLayers.indexOf(S.turbLayer);
    S.satelliteIRLayer = S.viewer.imageryLayers.addImageryProvider(provider, idx);
  } else if (S.radarLayer) {
    const idx = S.viewer.imageryLayers.indexOf(S.radarLayer);
    S.satelliteIRLayer = S.viewer.imageryLayers.addImageryProvider(provider, idx);
  } else {
    S.satelliteIRLayer = S.viewer.imageryLayers.addImageryProvider(provider);
  }
  S.satelliteIRLayer.alpha = CONFIG.weatherOverlayOpacity / 100;
}

export function enableSatelliteIR() {
  if (S.satelliteIRLayer) return;
  addSatelliteIRLayer();
  CONFIG.satelliteIREnabled = true;
  console.log('[Satellite] GOES IR overlay enabled');
  // Auto-refresh every 10 minutes
  if (S.satelliteIRRefreshTimer) clearInterval(S.satelliteIRRefreshTimer);
  S.satelliteIRRefreshTimer = setInterval(refreshSatelliteIR, 10 * 60 * 1000);
}

export function disableSatelliteIR() {
  if (S.satelliteIRLayer) {
    S.viewer.imageryLayers.remove(S.satelliteIRLayer);
    S.satelliteIRLayer = null;
  }
  CONFIG.satelliteIREnabled = false;
  if (S.satelliteIRRefreshTimer) {
    clearInterval(S.satelliteIRRefreshTimer);
    S.satelliteIRRefreshTimer = null;
  }
  console.log('[Satellite] GOES IR overlay disabled');
}

export function refreshSatelliteIR() {
...
export function enableRadar() {
  if (S.radarLayer) return;
  const provider = makeRadarProvider();
  S.radarLayer = S.viewer.imageryLayers.addImageryProvider(provider);
  ...
  S.radarRefreshTimer = setInterval(refreshRadar, 5 * 60 * 1000);
}

export function disableRadar() {
  if (S.radarLayer) {
    S.viewer.imageryLayers.remove(S.radarLayer);
    S.radarLayer = null;
  }
  CONFIG.radarEnabled = false;
  if (S.radarRefreshTimer) {
    clearInterval(S.radarRefreshTimer);
    S.radarRefreshTimer = null;
  }
...
// Mercator Y helper
export function geoLatToMercY(latDeg) {
...
export async function fetchTurbImageDataUrl(level, dateSecs, opts) {
...
export function createTurbProviderFromDataUrl(dataUrl) {
...
export async function addTurbLayer(dateSecs) {
  ...
  for (const level of levelsToTry) {
    ...
    if (provider) {
      if (S.radarLayer) {
        const radarIdx = S.viewer.imageryLayers.indexOf(S.radarLayer);
        S.turbLayer = S.viewer.imageryLayers.addImageryProvider(provider, radarIdx);
      } else {
        S.turbLayer = S.viewer.imageryLayers.addImageryProvider(provider);
      }
      S.turbLayer.alpha = CONFIG.weatherOverlayOpacity / 100;
      // Remove old layer AFTER adding new one to avoid flash
      if (oldLayer) {
        S.viewer.imageryLayers.remove(oldLayer);
      }
...
export function removeTurbLayer() {
  _turbAddGen++; // Cancel any in-flight async addTurbLayer()
  if (S.turbLayer) {
    S.viewer.imageryLayers.remove(S.turbLayer);
    S.turbLayer = null;
  }
}
...
export async function addTurb3DLayers(dateSecs) {
  ...
  for (const { fl, dataUrl } of results) {
    ...
    S.turb3dEntities.push(entity);
  }
  console.log(`[Weather] Added ${S.turb3dEntities.length} 3D turbulence altitude surfaces`);
}

export function removeTurb3DLayers() {
  _turb3dGen++;
  for (const entity of S.turb3dEntities) {
    S.viewer.entities.remove(entity);
  }
  S.turb3dEntities.length = 0;
}

export function removePirepEntities() {
  for (const entity of S.pirepEntities) {
    S.viewer.entities.remove(entity);
  }
  S.pirepEntities.length = 0;
}

export function removeSigmetEntities() {
  for (const entity of S.sigmetEntities) {
    S.viewer.entities.remove(entity);
  }
  S.sigmetEntities.length = 0;
}

export function removeAirmetEntities() {
  for (const entity of S.airmetEntities) {
    S.viewer.entities.remove(entity);
  }
  S.airmetEntities.length = 0;
}
...
export function pirepCssColor(intensity) {
    ...
    if (resp && resp.ok) {
      ...
      if (data.features) {
        for (const f of data.features) {
          ...
          const camH = S.viewer.camera.positionCartographic ? S.viewer.camera.positionCartographic.height : 1e7;
          const pirepDisplayCond = S.acDisplayCond || new Cesium.DistanceDisplayCondition(0, computeHorizonDist(camH));
          const entity = S.viewer.entities.add({
...
export function _buildAirmetEntities(responses, targetArray, idPrefix) {
  let count = 0;
...
export async function fetchAirmets() {
  ...
  try {
    ...
    const data = await resp.json();
    const count = _buildAirmetEntities([data], S.airmetEntities, 'turb-airmet');
...
export async function fetchAirmetsForScrubbing() {
  ...
  try {
    ...
    for (const entity of S.airmetEntities) entity.show = false;
...
export function removeScrubAirmetEntities() {
  for (const entity of S._scrubAirmetEntities) {
    S.viewer.entities.remove(entity);
  }
  S._scrubAirmetEntities.length = 0;
  // Restore live AIRMET entity visibility
  for (const entity of S.airmetEntities) entity.show = true;
}

export function enablePireps() {
  CONFIG.pirepsEnabled = true;
  console.log('[Weather] PIREPs enabled');
  fetchPireps();
  if (S.pirepRefreshTimer) clearInterval(S.pirepRefreshTimer);
  S.pirepRefreshTimer = setInterval(() => {
    removePirepEntities();
    fetchPireps();
  }, 5 * 60 * 1000);
}

export function disablePireps() {
  removePirepEntities();
  CONFIG.pirepsEnabled = false;
  if (S.pirepRefreshTimer) {
    clearInterval(S.pirepRefreshTimer);
    S.pirepRefreshTimer = null;
  }
  console.log('[Weather] PIREPs disabled');
}

export function enableSigmets() {
  CONFIG.sigmetsEnabled = true;
  console.log('[Weather] SIGMETs enabled');
  fetchSigmets();
  if (S.sigmetRefreshTimer) clearInterval(S.sigmetRefreshTimer);
  S.sigmetRefreshTimer = setInterval(() => {
    removeSigmetEntities();
    fetchSigmets();
  }, 5 * 60 * 1000);
}

export function disableSigmets() {
  removeSigmetEntities();
  CONFIG.sigmetsEnabled = false;
  if (S.sigmetRefreshTimer) {
    clearInterval(S.sigmetRefreshTimer);
    S.sigmetRefreshTimer = null;
  }
  console.log('[Weather] SIGMETs disabled');
}

export function enableAirmets() {
  CONFIG.airmetsEnabled = true;
  console.log('[Weather] AIRMETs enabled');
  fetchAirmets();
  if (S.airmetRefreshTimer) clearInterval(S.airmetRefreshTimer);
  S.airmetRefreshTimer = setInterval(() => {
    removeAirmetEntities();
    fetchAirmets();
  }, 5 * 60 * 1000);
}

export function disableAirmets() {
  removeAirmetEntities();
  removeScrubAirmetEntities();
  CONFIG.airmetsEnabled = false;
  if (S.airmetRefreshTimer) {
    clearInterval(S.airmetRefreshTimer);
    S.airmetRefreshTimer = null;
  }
...
export function parseAltToFL(value) {
...
export function isWeatherEntityVisible(entity, timeMs, altitudeFL) {
...
// timeMs: null = skip time check; altitudeFL: null = skip altitude check.
export function filterAllWeather(timeMs, altitudeFL) {
  for (const entity of S.pirepEntities) {
  ...
  if (S._scrubAirmetEntities.length > 0) {
    for (const entity of S._scrubAirmetEntities) {
      entity.show = isWeatherEntityVisible(entity, timeMs, altitudeFL);
    }
  } else {
...
export function getSelectedAircraftFL() {
  if (!S.selectedIcao) return null;
  const ac = S.aircraft.get(S.selectedIcao);
  if (!ac) return null;
...
export function computeTurbLevelForFL(fl) {
...
export function updateLiveTurbLevel(altFL) {
...
export function pauseWeatherRefresh() {
  if (S.pirepRefreshTimer) { clearInterval(S.pirepRefreshTimer); S.pirepRefreshTimer = null; }
  if (S.sigmetRefreshTimer) { clearInterval(S.sigmetRefreshTimer); S.sigmetRefreshTimer = null; }
  if (S.airmetRefreshTimer) { clearInterval(S.airmetRefreshTimer); S.airmetRefreshTimer = null; }
  if (S.turbRefreshTimer) { clearInterval(S.turbRefreshTimer); S.turbRefreshTimer = null; }
  if (S.radarRefreshTimer) { clearInterval(S.radarRefreshTimer); S.radarRefreshTimer = null; }
  if (S.satelliteIRRefreshTimer) { clearInterval(S.satelliteIRRefreshTimer); S.satelliteIRRefreshTimer = null; }
...
export function resumeWeatherRefresh() {
  if (CONFIG.pirepsEnabled && !S.pirepRefreshTimer) {
    S.pirepRefreshTimer = setInterval(() => { removePirepEntities(); fetchPireps(); }, 5 * 60 * 1000);
  }
  if (CONFIG.sigmetsEnabled && !S.sigmetRefreshTimer) {
    S.sigmetRefreshTimer = setInterval(() => { removeSigmetEntities(); fetchSigmets(); }, 5 * 60 * 1000);
  }
  if (CONFIG.airmetsEnabled && !S.airmetRefreshTimer) {
    S.airmetRefreshTimer = setInterval(() => { removeAirmetEntities(); fetchAirmets(); }, 5 * 60 * 1000);
  }
  if (CONFIG.turbForecastEnabled && !S.turbRefreshTimer) {
    S.turbRefreshTimer = setInterval(refreshTurbForecast, 15 * 60 * 1000);
  }
  if (CONFIG.radarEnabled && !S.radarRefreshTimer) {
    S.radarRefreshTimer = setInterval(refreshRadar, 5 * 60 * 1000);
  }
  if (CONFIG.satelliteIREnabled && !S.satelliteIRRefreshTimer) {
    S.satelliteIRRefreshTimer = setInterval(refreshSatelliteIR, 10 * 60 * 1000);
  }
...
export function enableTurbForecast() {
  if (CONFIG.turb3D) {
    if (S.turb3dEntities.length > 0) return;
    addTurb3DLayers();
  } else {
    if (S.turbLayer) return;
    addTurbLayer();
  }
  console.log(`[Weather] GTG forecast enabled: ${CONFIG.turbulenceLevel} (3D: ${CONFIG.turb3D})`);
  if (S.turbRefreshTimer) clearInterval(S.turbRefreshTimer);
  S.turbRefreshTimer = setInterval(refreshTurbForecast, 15 * 60 * 1000);
}

export function disableTurbForecast() {
  removeTurbLayer();
  removeTurb3DLayers();
  if (S.turbRefreshTimer) {
    clearInterval(S.turbRefreshTimer);
    S.turbRefreshTimer = null;
  }
  console.log('[Weather] GTG forecast disabled');
}

export function refreshTurbForecast() {
...
export function refreshTurbLevel() {
  if (!CONFIG.turbForecastEnabled) return;
...
export function updateTurbForTimelineTime(timeMs) {
  if (!CONFIG.turbForecastEnabled) return;
...
export function resetTurbToLive() {
...
export async function decodeDataUrlToImageData(dataUrl) {
...
export function getTurbPixelColor(lon, lat, dateSecs) {
...
export async function preloadTurbForTimeline(depMs, arrMs, onComplete) {
...
export function clearTurbCache() {
