...
export function removeTrailEntities(ac) {
  ...
  if (ac.extrapolationTrail) {
    S.viewer.entities.remove(ac.extrapolationTrail);
    ac.extrapolationTrail = null;
  }
}

export function refreshAllEntities() {
  S.viewer.entities.suspendEvents();
  try {
    for (const [icao, ac] of S.aircraft) {
      if (ac.entity) { S.viewer.entities.remove(ac.entity); ac.entity = null; }
      ac._iconKey = '';
      ac._labelText = '';
...
export function toggleAircraft(show) {
...
export function setTickInterval(ms) {
  ...
  S.tickTimer = setInterval(tick, ms);
}
...
export async function pollSelectedAircraft() {
  if (S._selectedPollInFlight) return;
  if (!S.selectedIcao) return;
  const ac = S.aircraft.get(S.selectedIcao);
  if (!ac) return;
  const now = Date.now();
  if (now - S._lastSelectedPollApiMs < RATE_LIMIT_MS) return;
  S._selectedPollInFlight = true;
  S._lastSelectedPollApiMs = now;
  const s = ac.state;
  const pad = 1;
  const bounds = {
  ...
  try {
    console.log(`[Poll] Selected aircraft poll for ${S.selectedIcao}...`);
    const t0 = Date.now();
    ...
    if (data.error) {
      if (data.retryIn) {
        ...
        S._lastSelectedPollApiMs = Date.now() - RATE_LIMIT_MS + data.retryIn;
    ...
    if (!data.error && data.states) {
      ...
      for (const raw of data.states) {
        ...
        if (ns.icao24 !== S.selectedIcao) continue;
        ...
        updateExtrapolationTrail(S.selectedIcao, ac, currentPos);
        break;
      }
      renderAircraft(new Set([S.selectedIcao]));
      showAircraftInfo(S.selectedIcao);
...
export function tick() {
  ...
  if (S.selectedIcao && now - S.lastSelectedPollMs >= SELECTED_POLL_INTERVAL) {
    S.lastSelectedPollMs = now;
    const ac = S.aircraft.get(S.selectedIcao);
    const coveredByBulk = CONFIG.aircraftEnabled && ac && S.lastPollBounds
      && ac.state.lat >= S.lastPollBounds.south && ac.state.lat <= S.lastPollBounds.north
      && ac.state.lon >= S.lastPollBounds.west && ac.state.lon <= S.lastPollBounds.east;
    if (!coveredByBulk) {
  ...
  if (S.trackFetchQueue.length > 0 && now - S.lastTrackFetchMs >= TRACK_FETCH_INTERVAL) {
    S.lastTrackFetchMs = now;
    fetchNextTrack();
  }
}
...
export function getViewBounds() {
  ...
  if (!rect) {
    ...
    const topLeft = S.viewer.camera.pickEllipsoid(
      new Cesium.Cartesian2(0, 0), Cesium.Ellipsoid.WGS84
    );
    const bottomRight = S.viewer.camera.pickEllipsoid(
...
export function updateAircraft(states) {
  const now = Date.now() / 1000;
  const seen = new Set();
  ...
  try {
    for (const raw of states) {
      ...
      if (s.icao24 !== S.selectedIcao && ac.granularTrack && ac.granularTrack.path) {
    ...
    for (const [icao, ac] of S.aircraft) {
      if (!seen.has(icao) && icao !== S.searchedIcao && icao !== S.selectedIcao) {
        ...
        if (age > CONFIG.staleThreshold) {
          if (ac.entity) S.viewer.entities.remove(ac.entity);
          removeTrailEntities(ac);
          S.aircraft.delete(icao);
        }
      }
    }
  } finally {
  ...
  if (S.selectedIcao && S.aircraft.has(S.selectedIcao)) {
    showAircraftInfo(S.selectedIcao);
    updateLiveAltitudeFilter();
  } else if (S.selectedIcao && !S.aircraft.has(S.selectedIcao)) {
...
export function updateExtrapolationTrail(icao, ac, currentPos) {
  // Selected aircraft always get extrapolation trail regardless of trail mode
  if (CONFIG.trailMode !== 'history' && icao !== S.selectedIcao) return;
  ...
  const isSelected = icao === S.selectedIcao;
  ...
  if (ac.extrapolationTrail) {
    ...
    ac.extrapolationTrail = S.viewer.entities.add({
      polyline: {
        positions: positions,
        width: trailWidth,
        material: material,
        clampToGround: false,
        distanceDisplayCondition: S.acDisplayCond,
      },
    });
  }
}
...
export function computeExtrapolatedPosition(s, baseTime, now) {
...
export function updateInfoPanelInterim(newPos) {
...
export function extrapolatePositions() {
  ...
  for (const [icao, ac] of S.aircraft) {
    ...
    const isSelectedAc = icao === S.selectedIcao;
...
export function computeHorizonDist(camHeight) {
  const R = 6371000; // Earth radius in meters
...
export function _renderOneAircraft(icao, ac, camHeight, useDot, showLabels) {
    ...
    if (!ac.entity) {
      ac.entity = S.viewer.entities.add({
        id: `ac-${icao}`,
        position: pos,
        billboard: {
          image: iconImage,
      ...
      if (S.isTracking && icao === S.selectedIcao) {
        S.viewer.trackedEntity = ac.entity;
      }
    } else {
      // Update position
    ...
    if (CONFIG.trailMode !== 'none' || isSelected) {
      ...
      if (!isSelected && CONFIG.trailMode === 'velocity' && s.heading != null && s.velocity != null) {
        ...
        if (lineLength > 100) {
          ...
          ac.trailEntities.push(S.viewer.entities.add({
            polyline: {
              positions: positions,
              width: trailWidth,
              material: material,
...
export function renderAircraft(filterIcaos) {
  ...
  function renderChunk() {
    ...
    if (idx < entries.length && gen === S._renderGeneration) {
...
export function resizeAircraftIcons() {
  const camHeight = S.viewer.camera.positionCartographic
    ? S.viewer.camera.positionCartographic.height : 0;
  const iconSize = computeDisplaySize(camHeight);
  const showLabels = CONFIG.labelsEnabled && camHeight < 800000;
  S.acDisplayCond = new Cesium.DistanceDisplayCondition(0, computeHorizonDist(camHeight));
  ...
  try {
    for (const [icao, ac] of S.aircraft) {
      ...
      if (ac.entity.billboard) {
        ac.entity.billboard.width = iconSize;
        ac.entity.billboard.height = iconSize;
        ac.entity.billboard.distanceDisplayCondition = S.acDisplayCond;
      }
      if (ac.entity.label) {
        ac.entity.label.show = (icao === S.selectedIcao) || showLabels;
...
export function smoothTrailPositions(points) {
...
export function buildTrailPositions(ac, isSelected = false) {
  const now = Date.now() / 1000;
...
export function padBounds(bounds, fraction) {
...
export async function pollStates() {
  ...
  const now = Date.now();
  if (now - S._lastBulkPollMs < RATE_LIMIT_MS) {
    console.log(`[Poll] Skipped: rate limit (${Math.round((RATE_LIMIT_MS - (now - S._lastBulkPollMs)) / 1000)}s remaining)`);
    return;
  }
  S._pollInFlight = true;
  ...
  try {
    ...
    S.lastPollBounds = bounds;
    document.getElementById('track-count').textContent = stateCount;
    document.getElementById('last-update').textContent =
      S.lastPollTime.toLocaleTimeString('en-US', { hour12: false });
...
export async function fetchNextTrack() {
  if (S.trackFetchQueue.length === 0) return;

  const icao24 = S.trackFetchQueue.shift();
  const ac = S.aircraft.get(icao24);
  if (!ac) return;
