// Aircraft selection, flight plan search, and route display.
...
export function estimateArrivalTime(flight) {
...
export function updateInfoPanelRoute(flight) {
...
export function showAircraftInfo(icao) {
  const ac = S.aircraft.get(icao);
  if (!ac) return;

  const prevSelected = S.selectedIcao;
  S.selectedIcao = icao;
  ...
  if (prevSelected !== icao) {
    ...
    try {
      for (const rid of toRefresh) {
        const rac = S.aircraft.get(rid);
        if (rac) {
          if (rac.entity) { S.viewer.entities.remove(rac.entity); rac.entity = null; }
    ...
    } else if (S.activeFlightPlan && S.activeFlightPlan.flights) {
      // Already have flight plan data from a prior search — show route timing
      const flight = pickBestFlight(S.activeFlightPlan.flights);
      if (flight) {
...
export function showTurbInfo(entity) {
  ...
  if (S.selectedIcao) {
    ...
    S.viewer.entities.suspendEvents();
    try {
      const rac = S.aircraft.get(prevIcao);
      if (rac) {
        if (rac.entity) { S.viewer.entities.remove(rac.entity); rac.entity = null; }
        rac._iconKey = ''; rac._labelText = '';
        removeTrailEntities(rac);
...
export function hideAircraftInfo() {
  stopTracking();
  const prevIcao = S.selectedIcao;
  S.selectedIcao = null;
...
export async function enrichSelectedWithFlightAware(icao, callsign) {
  ...
  try {
    ...
    if (result && result.flight) {
      S.selectedRouteFlight = result.flight;
...
export function pickBestFlight(flights) {
  const now = new Date();
...
export function flyToRouteOverview() {
  if (S.flightPlanEntities.length === 0) return;
  S.viewer.flyTo(S.flightPlanEntities, {
    duration: 1.5,
    offset: new Cesium.HeadingPitchRange(0, -Math.PI / 2, 0),
  });
}

export function clearFlightPlanRoute() {
  if (typeof hideTimeline === 'function') hideTimeline();
  S.viewer.entities.suspendEvents();
  try {
    for (const e of S.flightPlanEntities) S.viewer.entities.remove(e);
  } finally {
    S.viewer.entities.resumeEvents();
  }
  S.flightPlanEntities.length = 0;
  S.timelineRoutePoints.length = 0;
  S.activeFlightPlan = null;
  S.selectedRouteFlight = null;
  S.searchedFlightIdent = null;
  S.searchedIcao = null;
  refreshTurbLevel();
}
...
export function drawFlightPlanMarkers(origin, dest, originCoords, destCoords, waypointColor) {
  S.viewer.entities.suspendEvents();
  try {
    if (originCoords) {
      const originLabel = (origin && (origin.code_iata || origin.code_icao || origin.code)) || 'DEP';
      S.flightPlanEntities.push(S.viewer.entities.add({
...
export function displayFlightPlanRoute(flightData, preSelectedFlight = null) {
  clearFlightPlanRoute();
  S.activeFlightPlan = flightData;
  refreshTurbLevel();
  ...
  if (S.flightPlanEntities.length > 0) {
    flyToRouteOverview();
  }
...
export async function findAndSelectViaOpenSky(flight, originCoords, destCoords) {
...
export async function fetchAndDisplayFiledRoute(faFlightId, flight, originCoords, destCoords, routeColor, waypointColor, cruiseAltMeters) {
      ...
      if (!data.error) {
        ...
        if (validFixes.length > 0) {
          ...
          if (originCoords) {
            ...
            S.timelineRoutePoints.push({ lon: originCoords.lon, lat: originCoords.lat, alt: 0 });
          }
          for (const fix of validFixes) {
            routePositions.push(Cesium.Cartesian3.fromDegrees(fix.longitude, fix.latitude, exAlt(alt)));
            S.timelineRoutePoints.push({ lon: fix.longitude, lat: fix.latitude, alt });
          }
          if (destCoords) {
            routePositions.push(Cesium.Cartesian3.fromDegrees(destCoords.lon, destCoords.lat, 0));
            S.timelineRoutePoints.push({ lon: destCoords.lon, lat: destCoords.lat, alt: 0 });
          }
...
export function drawRouteFromString(routeStr, originCoords, destCoords, routeColor, waypointColor, cruiseAltMeters) {
  ...
  for (const wp of routeWaypoints) {
    ...
    S.timelineRoutePoints.push({ lon: wp.lon, lat: wp.lat, alt });
  }
  if (destCoords) {
    routePositions.push(Cesium.Cartesian3.fromDegrees(destCoords.lon, destCoords.lat, 0));
    S.timelineRoutePoints.push({ lon: destCoords.lon, lat: destCoords.lat, alt: 0 });
...
export function showFlightResults(flights, flightData) {
...
export function hideFlightResults() {
  const panel = document.getElementById('flight-results');
...
export async function selectFlightFromResults(flight, flightData) {
...
export function parseHourStr(timeStr, ampm) {
...
export async function searchFlightsByNL(query) {
...
export async function searchFlightPlan(ident) {
...
export async function addSearchHistory(ident) {
...
if (trackBtn) {
  trackBtn.addEventListener('click', () => {
    ...
    const heading = S.viewer.camera.heading;
    const range = 200000;
    ...
    S.viewer.flyTo(ac.entity, {
      duration: 1.5,
      offset: new Cesium.HeadingPitchRange(heading, -pitchRad, range),
    }).then(() => {
      if (S.isTracking && S.selectedIcao) {
        const currentAc = S.aircraft.get(S.selectedIcao);
        if (currentAc && currentAc.entity) {
          S.viewer.trackedEntity = currentAc.entity;
        }
      }
    });
  });
  ...
  showRouteBtn.addEventListener('click', () => {
    ...
    if (S.flightPlanEntities.length > 0) {
      ...
      if (ac && ac.trailEntities && ac.trailEntities.length > 0) {
        S.viewer.flyTo(ac.trailEntities, {
          duration: 1.5,
          offset: new Cesium.HeadingPitchRange(0, -Math.PI / 2, 0),
        });
      }
