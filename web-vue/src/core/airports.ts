/**
 * Built-in airport database (major US airports + select international).
 * Ported from shared/data.js lines 10-68.
 *
 * Note: The full airport dataset is loaded at runtime from airports.json.
 * This module provides the hardcoded subset used for quick lookups
 * (e.g. resolving departure/destination codes without a network request).
 */

import type { Airport, AirportDict } from './types';

export const AIRPORTS: AirportDict = {
  // Northeast
...
/** Look up an airport by ICAO or IATA code (case-insensitive) */
