...
let _rainViewerMaps = null;       // cached RainViewer maps JSON { host, radar: { past:[], nowcast:[] } }
let _rainViewerFetchMs = 0;       // timestamp of last RainViewer maps fetch
let _radarLoopTimer = null;       // interval ID for radar loop animation
let _radarLoopIdx = 0;            // current frame index in radar loop
let _radarLoopFrames = [];        // ordered list of { time, path } for loop playback
let _radarLoopPlaying = false;    // true while radar loop animation is running
