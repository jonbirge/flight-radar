...
- **`shared/data.js`**: Airport database, airplanes.live state parsing (`parseState`), and data block formatting.
...
- **Main process** (`main.js`): airplanes.live API calls, FlightAware AeroAPI calls, rate limiting, settings persistence (`settings.json` in userData dir), and window/menu management.
...
- **`web/app.js`**: Web-specific entry point — `window.flightAPI` shim (localStorage settings, airplanes.live API via `fetch()`), settings panel wiring via `shared/settings.js`, and init.
...
- **`web/cred.php`**: Deprecated (was OpenSky OAuth2 token proxy, no longer needed).
...
- **API proxies**: Browsers enforce CORS, so the web version routes FlightAware and AWC API calls through PHP proxies (`flightaware-proxy.php`, `awc-proxy.php`). airplanes.live supports CORS and is called directly. Electron makes direct HTTPS calls from the main process. The `window.flightAPI` abstraction hides this difference from shared code.
