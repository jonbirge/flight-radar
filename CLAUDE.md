...
Build system: **electron-vite** (Vite-based Electron tooling). Config in `electron.vite.config.mjs`. Test config in `vitest.config.js`. Packaging via **electron-builder** (config in `package.json` `"build"` field). The build requires `vendor/cesium/` and `vendor/fonts/` which are created by postinstall — run `npm install` before first build. Postinstall also patches the Electron.app `Info.plist` on macOS so the menu bar shows "Flight Radar" instead of "Electron" during development. Application icons (`assets/icon.png`, `icon.ico`, `icon.icns`) are generated by `scripts/generate-icon.js` as part of the build chain (via `prebuild`/`predev` hooks) — the `assets/` directory is gitignored and created fresh each build.
...
- **Vite build**: Modules are ES modules bundled by Vite via `electron-vite`. CesiumJS is an npm devDependency; a postinstall script copies `Build/Cesium/` to `vendor/cesium/`. A second postinstall script (`scripts/check-fonts.js`) verifies/downloads fonts to `vendor/fonts/`. The `vite-plugin-static-copy` copies vendor, data, and font files to the build output.
- **Font loading**: Fonts are bundled locally in `vendor/fonts/`. `src/index.html`, `src/settings.css`, and `src/help.css` each declare `@font-face` rules pointing to `../vendor/fonts/`. Do NOT put a `@font-face` in `src/styles.css` — it is imported by `entry.js` and the relative path would not resolve correctly.
...
- **Versioning**: Do not bump the version automatically on every commit. Version bumps should only be done when explicitly requested. Do not bump the version on feature branches — this avoids merge conflicts. Uses semantic versioning (MAJOR.MINOR.PATCH): bump the first digit for major features or breaking changes, the second digit for minor features, and the third digit for bug fixes.
