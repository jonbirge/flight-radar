...
Build system: **electron-vite** (Vite-based Electron tooling). Config in `electron.vite.config.mjs`. Test config in `vitest.config.js`. Packaging via **electron-builder** (config in `package.json` `"build"` field). The build requires `vendor/cesium/` and `src/fonts/` which are created by postinstall — run `npm install` before first build. Postinstall also patches the Electron.app `Info.plist` on macOS so the menu bar shows "Flight Radar" instead of "Electron" during development. Application icons (`assets/icon.png`, `icon.ico`, `icon.icns`) are generated by `scripts/generate-icon.js` as part of the build chain (via `prebuild`/`predev` hooks) — the `assets/` directory is gitignored and created fresh each build.
